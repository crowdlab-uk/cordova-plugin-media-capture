...
- (void)viewDidLoad {
  ...
  UIButton *cameraIconView = [UIButton buttonWithType:UIButtonTypeCustom];
  [cameraIconView setFrame:CGRectMake(0, 0, 900, 900)];

  cameraIconView.translatesAutoresizingMaskIntoConstraints = NO;
  cameraIconView.layer.frame = cameraIconView.bounds;

  CALayer *parentLayer = [CALayer new];
  parentLayer.backgroundColor = [[UIColor blueColor] CGColor];
  [cameraIconView.layer addSublayer:parentLayer];

  UIImage *switchImage = [UIImage imageNamed:@"camera-switch-crowdlab"];
  switchImage = [switchImage imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];

  UIImageView *imv = [[UIImageView alloc] initWithImage:switchImage];
  ...
  [cameraIconView addSubview:imv];

  imv.tintColor = UIColor.systemBlueColor;

  UIBarButtonItem *switchButton = [[UIBarButtonItem alloc] initWithCustomView:imv];
